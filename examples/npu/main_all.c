    ...
    )
{
    ...
    if(fileType == NN_FILE_JPG)
    {
...

        {
            ...
            int32_t crop_length[] = {static_cast<int32_t>(attr.size[0]), static_cast<int32_t>(attr.size[1]), 3, 1};
            int32_t resize_length[] = {static_cast<int32_t>(tensor->attr.size[0]), static_cast<int32_t>(tensor->attr.size[1])};
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    if( use_img_process_s )
    {
    	std::cerr<<"use image process s\n";
        enable_pre_post_process = atoi(use_img_process_s);
    }
    if( enable_pre_post_process )
    {
    	std::cerr<<"enable pre post process\n";
        if( pre_process_map_count > 0 )
        {
            std::cerr<<"preprocess map count:"<<pre_process_map_count<<std::endl;
        ...
        {
            std::cerr<<"postprocess map count:"<<post_process_map_count<<std::endl;
